...
def test_readme_sheet_contains_required_sections():
    """README sheet should include title, disclaimer, demo paths, and talking points."""
    wb = Workbook()
    ws = wb.active
    ws.title = "README"
    ...
    assert "Inventory Optimization Demo Path" in combined
    assert "IT Inventory Control Demo Path" in combined
    assert "Key Capabilities" in combined
    assert "Interview Talking Points" in combined
    assert "Inventory Dashboard" in combined
    assert "IT Asset Register" in combined
