"""
Workbook builder: orchestrates data loading and sheet creation.

Creates Inventory_IT_Asset_Control_Toolkit.xlsx in dist/.
"""

import logging
from pathlib import Path
from typing import Any

import pandas as pd
from openpyxl import Workbook

from config.workbook_config import (
    AUTHOR,
    SHEET_ORDER,
    WORKBOOK_PATH,
    WORKBOOK_TITLE,
)
from src.sheets import (
    aged_excess_sheet,
    audit_reconciliation_sheet,
    disposal_log_sheet,
    inventory_dashboard_sheet,
    it_asset_register_sheet,
    management_summary_sheet,
    markdown_planner_sheet,
    master_inventory_sheet,
    mobile_provisioning_sheet,
    readme_sheet,
    software_licenses_sheet,
    transfer_planner_sheet,
)

logger = logging.getLogger(__name__)

# Maps sheet name → builder function (called in SHEET_ORDER sequence)
SHEET_BUILDERS = {
    "README": readme_sheet.build,
    "Master Inventory": master_inventory_sheet.build,
    "Inventory Dashboard": inventory_dashboard_sheet.build,
    "Aged Excess Analysis": aged_excess_sheet.build,
    "Markdown Planner": markdown_planner_sheet.build,
    "Transfer Planner": transfer_planner_sheet.build,
    "IT Asset Register": it_asset_register_sheet.build,
    "Audit Reconciliation": audit_reconciliation_sheet.build,
    "Software Licenses": software_licenses_sheet.build,
    "Mobile Provisioning": mobile_provisioning_sheet.build,
    "Disposal Log": disposal_log_sheet.build,
    "Management Summary": management_summary_sheet.build,
}


def create_workbook() -> Workbook:
    """
    Create a new openpyxl Workbook with the default sheet removed.

    Returns:
        Empty Workbook ready for sheet builders.
    """
    wb = Workbook()
    default_ws = wb.active
    if default_ws is not None:
        wb.remove(default_ws)
    wb.properties.title = WORKBOOK_TITLE
    wb.properties.creator = AUTHOR
    wb.properties.subject = "Inventory & IT Asset Control"
    return wb


def build_workbook(data: dict[str, pd.DataFrame], output_path: Path | None = None) -> Path:
    """
    Assemble the full workbook from generated DataFrames.

    Creates a new workbook, builds each worksheet in required order,
    and saves to dist/Inventory_IT_Asset_Control_Toolkit.xlsx.

    Args:
        data: Dict with keys inventory, it_assets, software, mobile, disposal.
        output_path: Override default dist/ output path.

    Returns:
        Path to the saved .xlsx file.
    """
    output_path = output_path or WORKBOOK_PATH
    output_path.parent.mkdir(parents=True, exist_ok=True)

    wb = create_workbook()
    context: dict[str, Any] = {"data": data, "workbook": wb}

    for sheet_name in SHEET_ORDER:
        builder = SHEET_BUILDERS[sheet_name]
        ws = wb.create_sheet(title=sheet_name)
        logger.debug("Building sheet: %s", sheet_name)
        builder(ws, context)

    wb.save(output_path)
    logger.info("Workbook saved: %s", output_path)
    return output_path
