...
from config.workbook_config import INVENTORY_STATUSES, INVENTORY_THRESHOLDS, RECOMMENDED_ACTIONS
from src.workbook.charts import add_bar_chart, add_pie_chart, make_category_reference, make_data_reference
from src.workbook.formulas import average_range, count_if_range, sum_if_numeric, sum_if_range, sum_range
from src.workbook.styles import apply_kpi_card_style, apply_section_header_style, apply_table_header_style
...
def _recovery_value_formula(start: int, end: int) -> str:
    ...
    parts = [sum_if_range(MASTER_SHEET, COL_RECOMMENDED_ACTION, action, COL_TOTAL_VALUE, start, end)
...
def _kpi_definitions(start: int, end: int) -> list[tuple[str, str, str]]:
    ...
    return [
        ("Total Inventory Value", sum_range(MASTER_SHEET, COL_TOTAL_VALUE, start, end), "currency"),
        (
            "Aged Inventory Value",
            sum_if_numeric(
                MASTER_SHEET, COL_AGE_DAYS, f">{aged_threshold}", COL_TOTAL_VALUE, start, end
            ),
            "currency",
        ),
        (
            ...
            "="
            + "+".join(
                sum_if_range(MASTER_SHEET, COL_STATUS, status, COL_TOTAL_VALUE, start, end).lstrip("=")
        ...
        (
            ...
            "="
            + "+".join(
                count_if_range(MASTER_SHEET, COL_RECOMMENDED_ACTION, start, end, action).lstrip("=")
...
def _write_kpi_cards(ws: Worksheet, start: int, end: int) -> None:
    ...
    for idx, (title, formula, fmt) in enumerate(kpis):
        ...
        apply_kpi_card_style(ws, title_row, col, value_row, col, title, formula, span_cols=2)
...
def _compute_status_summary(df: pd.DataFrame) -> pd.DataFrame:
    ...
    summary = (
        ...
        .agg(sku_count=("item_id", "count"), inventory_value=("total_value", "sum"))
    )
...
def _compute_action_summary(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if df.empty:
        return pd.DataFrame(columns=["recommended_action", "sku_count", "inventory_value"])
    summary = (
        df.groupby("recommended_action", as_index=False)
        .agg(sku_count=("item_id", "count"), inventory_value=("total_value", "sum"))
    )
...
def _compute_top_excess(df: pd.DataFrame, limit: int = 10) -> pd.DataFrame:
    ...
    return top[["sku", "product_name", "location", "excess_qty", "excess_value"]].reset_index(drop=True)
...
def _write_summary_tables(
    ...
    meta["location"] = {"header_row": row + 1, "first_data_row": first, "last_data_row": last, "value_col": 2}
    row = last + 3
    ...
    meta["status"] = {"header_row": row + 1, "first_data_row": first, "last_data_row": last, "value_col": 2}
    row = last + 3
    ...
    meta["aging"] = {"header_row": row + 1, "first_data_row": first, "last_data_row": last, "value_col": 2}
    row = last + 3
    ...
    meta["action"] = {"header_row": row + 1, "first_data_row": first, "last_data_row": last, "value_col": 2}
    row = last + 3
    ...
    meta["top_excess"] = {"header_row": row + 1, "first_data_row": first, "last_data_row": last, "value_col": 5}
    return meta
...
def _format_summary_tables(ws: Worksheet, meta: dict[str, dict[str, int]]) -> None:
    ...
    if status_info["last_data_row"] >= status_info["first_data_row"]:
        format_integer_columns(ws, ["B"], status_info["first_data_row"], status_info["last_data_row"])
        format_currency_columns(ws, ["C"], status_info["first_data_row"], status_info["last_data_row"])

    aging = meta["aging"]
    if aging["last_data_row"] >= aging["first_data_row"]:
        format_integer_columns(ws, ["B"], aging["first_data_row"], aging["last_data_row"])
        format_currency_columns(ws, ["C"], aging["first_data_row"], aging["last_data_row"])

    action = meta["action"]
    if action["last_data_row"] >= action["first_data_row"]:
        format_integer_columns(ws, ["B"], action["first_data_row"], action["last_data_row"])
        format_currency_columns(ws, ["C"], action["first_data_row"], action["last_data_row"])

    excess = meta["top_excess"]
    if excess["last_data_row"] >= excess["first_data_row"]:
        format_integer_columns(ws, ["D"], excess["first_data_row"], excess["last_data_row"])
        format_currency_columns(ws, ["E"], excess["first_data_row"], excess["last_data_row"])
...
def _add_dashboard_charts(ws: Worksheet, meta: dict[str, dict[str, int]]) -> None:
    ...
    loc = meta["location"]
    if loc["last_data_row"] >= loc["first_data_row"]:
        cats = make_category_reference(ws, 1, loc["first_data_row"], loc["last_data_row"])
    ...
    if status["last_data_row"] >= status["first_data_row"]:
        cats = make_category_reference(ws, 1, status["first_data_row"], status["last_data_row"])
    ...
    aging = meta["aging"]
    if aging["last_data_row"] >= aging["first_data_row"]:
        cats = make_category_reference(ws, 1, aging["first_data_row"], aging["last_data_row"])
    ...
    excess = meta["top_excess"]
    if excess["last_data_row"] >= excess["first_data_row"]:
        cats = make_category_reference(ws, 1, excess["first_data_row"], excess["last_data_row"])
