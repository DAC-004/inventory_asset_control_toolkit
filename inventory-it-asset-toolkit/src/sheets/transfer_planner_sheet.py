...
from src.workbook.styles import apply_risk_conditional_formatting, apply_table_header_style, freeze_panes
...
def _estimate_transfer_cost(source_location: str, destination_location: str, quantity: int) -> float:
    """
...
def _destination_needs_stock(dest: pd.Series) -> bool:
    ...
    return (below_min and dest["demand_90_day"] > 0) or strong_demand
...
def _suggested_transfer_quantity(
    ...
    demand_need = max(int(dest["demand_90_day"] // 4), 0) if dest["demand_90_day"] >= STRONG_DEMAND_THRESHOLD else 0
...
def _apply_net_benefit_formatting(ws: Worksheet, last_row: int) -> None:
    ...
    green_fill = PatternFill(start_color=sc.COLORS["green"], end_color=sc.COLORS["green"], fill_type="solid")
...
def _apply_recommendation_formatting(ws: Worksheet, last_row: int) -> None:
    ...
    recommendation_range = f"{COL_RECOMMENDATION}{DATA_START_ROW}:{COL_RECOMMENDATION}{last_row}"
