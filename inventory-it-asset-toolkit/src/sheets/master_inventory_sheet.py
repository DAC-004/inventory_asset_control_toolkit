...
def _apply_status_formatting(ws: Worksheet, last_row: int) -> None:
    ...
    action_range = f"{COL_RECOMMENDED_ACTION}{DATA_START_ROW}:{COL_RECOMMENDED_ACTION}{last_row}"

    apply_inventory_status_formatting(
        ws, status_range, COL_STATUS, DATA_START_ROW
    )
    apply_risk_conditional_formatting(
        ws,
        action_range,
...
def _apply_validations(ws: Worksheet, last_row: int) -> None:
    ...
    add_inventory_status_validation(ws, f"{COL_STATUS}{DATA_START_ROW}:{COL_STATUS}{last_row}")
    add_recommended_action_validation(
